...
class BudgetManagerPanel extends HTMLElement {
    ...
    const renderGroup = (group) => {
      ...
      return `<tr class="matrix-group ${group.kind}"><th colspan="${months.length + 1}"><span class="kind-dot"></span>${group.label}</th></tr>${groupRows.map((row) => {
        ...
        const name = editableName
          ? `<span class="plan-row-label editing"><span class="kind-dot"></span><input class="plan-name-input" type="text" value="${this._esc(row.name)}" data-action="edit-plan-name" data-kind="${row.kind}" data-original-name="${this._esc(row.name)}" aria-label="Rename ${this._esc(row.name)}" required></span>`
          : `<span class="plan-row-label"><span class="kind-dot"></span>${this._esc(row.name)}</span>`;
        ...
        <th title="${this._esc(row.name)}"><div class="plan-row-heading">${name}${controls}</div></th>
  ...
  _bindEvents() {
    this.shadowRoot.querySelectorAll('[data-action]:not([data-action="edit-matrix-value"]):not([data-action="edit-plan-name"])').forEach((node) => {
    ...
    this.shadowRoot.querySelectorAll('[data-action="edit-plan-name"]').forEach((input) => {
      input.addEventListener("change", (event) => this._savePlanName(event.currentTarget));
      input.addEventListener("keydown", (event) => {
        if (event.key === "Enter") { event.preventDefault(); event.currentTarget.blur(); }
  ...
  async _savePlanName(input) {
    if (input.dataset.saving === "true") return;
    const oldName = String(input.dataset.originalName || "").trim();
    const newName = input.value.trim();
    if (!newName) {
      input.value = oldName;
      return this._showError(new Error("Item name is required."));
    }
    if (newName === oldName) return;
    input.dataset.saving = "true";
    input.disabled = true;
    try {
      const renamed = await this._hass.callWS({
      ...
      this._state = state;
      this._year = state.selected_year;
      this._showMessage(`Renamed ${renamed.updated_count} ${renamed.updated_count === 1 ? "occurrence" : "occurrences"}.`);
      this._render();
    } catch (err) {
      input.disabled = false;
      input.dataset.saving = "false";
  ...
  _initPlanSortable() {
    if (!this._matrixEditMode || !this._canEdit || this._planSortable) return;
    const body = this.shadowRoot.querySelector(".matrix tbody");
    if (!body || !body.querySelector(".plan-drag-handle")) return;
    this._planSortable = Sortable.create(body, {
      animation: 150,
      handle: ".plan-drag-handle",
      draggable: "tr[data-plan-kind][data-plan-order-name]",
      ghostClass: "plan-sort-ghost",
      chosenClass: "plan-sort-chosen",
      dragClass: "plan-sort-drag",
      fallbackClass: "plan-sort-fallback",
      fallbackTolerance: 3,
      scroll: true,
      forceAutoScrollFallback: true,
      scrollSensitivity: 56,
      scrollSpeed: 20,
      onMove: (event) => event.dragged.dataset.planKind === event.related?.dataset?.planKind,
      onStart: (event) => {
        const kind = event.item.dataset.planKind;
        this._sortablePlanDrag = { kind, originalOrder: this._planRowOrder(kind) };
      },
      onEnd: (event) => {
        const drag = this._sortablePlanDrag;
        this._sortablePlanDrag = null;
        event.item.classList.remove("plan-sort-ghost", "plan-sort-chosen", "plan-sort-drag", "plan-sort-fallback");
  ...
  _destroyPlanSortable() {
    this._planSortable?.destroy();
    this._planSortable = null;
    this._sortablePlanDrag = null;
  }
